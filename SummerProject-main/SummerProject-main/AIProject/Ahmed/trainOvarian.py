...
def file2matrix():
    df = pd.read_csv('../ovarian_cancer_data.csv')
    ...
    data = df.iloc[:, 2:-1].apply(pd.to_numeric)
    labels = df['Level'].apply(pd.to_numeric)

    print("Label distribution: ", np.bincount(labels))

    return data.values, labels.values, df.columns[2:-1]
...
def predict_risk(model, scaler, attributes):
    print("Enter the following details to predict ovarian cancer risk:")
    user_input = []
...
if __name__ == '__main__':
    ...
    # Uncomment the following line if you want to enable risk prediction interaction
